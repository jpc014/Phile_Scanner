...
{
  // get last hash
	unsigned __int32 a = m_hash[0];
	unsigned __int32 b = m_hash[1];
	unsigned __int32 c = m_hash[2];
	unsigned __int32 d = m_hash[3];

  // data represented as 16x 32-bit words
  const unsigned __int32* words = (unsigned __int32*) data;
...
#define LITTLEENDIAN(x) (x)
  ...
  // first round
  unsigned __int32 word0  = LITTLEENDIAN(words[ 0]);
  a = rotate(a + f1(b,c,d) + word0  + 0xd76aa478,  7) + b;
  unsigned __int32 word1  = LITTLEENDIAN(words[ 1]);
  d = rotate(d + f1(a,b,c) + word1  + 0xe8c7b756, 12) + a;
  unsigned __int32 word2  = LITTLEENDIAN(words[ 2]);
  c = rotate(c + f1(d,a,b) + word2  + 0x242070db, 17) + d;
  unsigned __int32 word3  = LITTLEENDIAN(words[ 3]);
  b = rotate(b + f1(c,d,a) + word3  + 0xc1bdceee, 22) + c;

  unsigned __int32 word4  = LITTLEENDIAN(words[ 4]);
  a = rotate(a + f1(b,c,d) + word4  + 0xf57c0faf,  7) + b;
  unsigned __int32 word5  = LITTLEENDIAN(words[ 5]);
  d = rotate(d + f1(a,b,c) + word5  + 0x4787c62a, 12) + a;
  unsigned __int32 word6  = LITTLEENDIAN(words[ 6]);
  c = rotate(c + f1(d,a,b) + word6  + 0xa8304613, 17) + d;
  unsigned __int32 word7  = LITTLEENDIAN(words[ 7]);
  b = rotate(b + f1(c,d,a) + word7  + 0xfd469501, 22) + c;

  unsigned __int32 word8  = LITTLEENDIAN(words[ 8]);
  a = rotate(a + f1(b,c,d) + word8  + 0x698098d8,  7) + b;
  unsigned __int32 word9  = LITTLEENDIAN(words[ 9]);
  d = rotate(d + f1(a,b,c) + word9  + 0x8b44f7af, 12) + a;
  unsigned __int32 word10 = LITTLEENDIAN(words[10]);
  c = rotate(c + f1(d,a,b) + word10 + 0xffff5bb1, 17) + d;
  unsigned __int32 word11 = LITTLEENDIAN(words[11]);
  b = rotate(b + f1(c,d,a) + word11 + 0x895cd7be, 22) + c;

  unsigned __int32 word12 = LITTLEENDIAN(words[12]);
  a = rotate(a + f1(b,c,d) + word12 + 0x6b901122,  7) + b;
  unsigned __int32 word13 = LITTLEENDIAN(words[13]);
  d = rotate(d + f1(a,b,c) + word13 + 0xfd987193, 12) + a;
  unsigned __int32 word14 = LITTLEENDIAN(words[14]);
  c = rotate(c + f1(d,a,b) + word14 + 0xa679438e, 17) + d;
  unsigned __int32 word15 = LITTLEENDIAN(words[15]);
...
void MD5::add(const char* data, size_t numBytes)
{
  const unsigned __int8* current = (const unsigned __int8*) data;
...
void MD5::processBuffer()
{
  ...
  unsigned __int64 msgBits = 8 * (m_numBytes + m_bufferSize);
  // find right position
...
MD5::MD5(const char* data, size_t numBytes)
{
	reset();
	add(data, numBytes);
}
